...
public class Login extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String passwordConvert = encrypt.toSHA1(password);
        AccountDBContext acc = new AccountDBContext();
        Account cus = acc.checkLogin(email, passwordConvert);
        if (cus == null) {
            request.setAttribute("error", "Wrong username or password!!");
            request.getRequestDispatcher("views/guest/login.jsp").forward(request, response);
        } else if(cus.isStatus()==false){
            request.setAttribute("error", "Account was banned");
            request.getRequestDispatcher("views/guest/login.jsp").forward(request, response);
        } else{
            HttpSession session = request.getSession();
            session.setAttribute("acc", cus);
            switch (acc.getRoleID(email)) {
                case 2:
