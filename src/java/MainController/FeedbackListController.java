...
public class FeedbackListController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Feedback> feedbacks = fbd.all();
        if(request.getParameter("currentpage")==null){
            request.setAttribute("pagination", new Pagination(feedbacks.size(), 1));
        }
        else{
            request.setAttribute("pagination", new Pagination(feedbacks.size(), Integer.parseInt(request.getParameter("currentpage"))));
        }
